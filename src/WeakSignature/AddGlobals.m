function yOffsetFinal=AddGlobals(address, yOffset, globalGotos, gotoLength, dgt)
	%  addGlobalGoto - A function that adds and repositions global gotos to
	%  the model
    %
	%	INPUTS
    %
	%	-address: the name and location in the model
	%	-yOffset: y point to start repositioning
	%	-globalGotos: names of all the global gotos to add
	%	-gotoLength: max length of global goto Tags
    
	%	OUTPUTS
    %
	%	-yOffsetFinal: y point after addGlobalGoto is done
	%
    num=0;
    termnum=0;
    if dgt == 0
